...
export class OperadoresService {
  ...
  async getKpisOperador(operadorId: number, fechaInicio?: string, fechaFin?: string) {
    ...
    return {
      ...
      rendimiento_tabulado_promedio: Number(kpis.rendimiento_tabulado_promedio) || 0,
  ...
  async getLiquidacionesOperador(operadorId: number, fechaInicio?: string, fechaFin?: string) {
    const qb = this.liquidacionesRepository
        .createQueryBuilder('l')
        .leftJoin('l.unidad', 'unidad')
        .select([
    ...
    if (fechaInicio) qb.andWhere('l.fecha_inicio >= :fechaInicio', { fechaInicio })
