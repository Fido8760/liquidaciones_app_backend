...
export class LiquidacionWorkflowService {
  ...
  async cambiarEstado(
    ...
    this.validacionesService.validarCambioEstado(
      estadoActual,
      nuevoEstado,
      user,
    );
    ...
    if (
      nuevoEstado === EstadoLiquidacion.APROBADA &&
      user.rol === UserRole.CAPTURISTA
    ) {
    ...
    if (
      estadoActual === EstadoLiquidacion.APROBADA &&
      nuevoEstado === EstadoLiquidacion.EN_REVISION
    ) {
    ...
    if (
      estadoActual === EstadoLiquidacion.PAGADA &&
      nuevoEstado !== EstadoLiquidacion.PAGADA
    ) {
  ...
  async ajustarLiquidacion(
    liquidacionId: number,
    ajustarLiquidacionDto: AjustarLiquidacionDto,
    user: User,
  ) {
    const liquidacion = await this.liquidacionesRepository.findOne({
      where: { id: liquidacionId },
    ...
    liquidacion.rendimiento_tabulado =
      ajustarLiquidacionDto.rendimiento_tabulado;
    ...
    const liquidacionActualizada = await this.calculoService.recalcularTotales(
      liquidacionId,
      user,
    );
  ...
  async modificarTotalPago(
    liquidacionId: number,
    modificarTotalDto: ModificarTotalDto,
    user: User,
  ) {
    const liquidacion = await this.liquidacionesRepository.findOne({
      where: { id: liquidacionId },
    ...
    const utilidadViaje =
      Number(liquidacion.total_fletes) -
