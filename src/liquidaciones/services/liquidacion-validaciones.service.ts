...
type TransitionMap = Partial<
  Record<UserRole, Partial<Record<EstadoLiquidacion, EstadoLiquidacion[]>>>
>;
...
export class LiquidacionValidacionesService {
  validarEstadoFinal(liquidacion: Liquidacion, user: User) {
    ...
    if (
      estadoFinales.includes(liquidacion.estado) &&
      user.rol !== UserRole.SISTEMAS
    ) {
      throw new ForbiddenException(
        `La liquidación está ${liquidacion.estado} y cerrada definitivamente.`,
      );
    }
  }
  ...
  validarCambioEstado(
    ...
    if (user.rol === UserRole.SISTEMAS) {
      if (nuevoEstado === EstadoLiquidacion.PAGADA) {
        throw new ForbiddenException(
          'Sistemas no tiene facultades para autorizar PAGOS.',
        );
      }
      return;
    }
    ...
    if (!permitidos.includes(nuevoEstado)) {
      throw new ForbiddenException(
        `No tienes permisos para cambiar de ${estadoActual} a ${nuevoEstado}.`,
      );
    }
  }
  ...
  validarAjuste(liquidacion: Liquidacion, user: User) {
    if (
      ![UserRole.DIRECTOR, UserRole.ADMIN, UserRole.SISTEMAS].includes(user.rol)
    ) {
      throw new ForbiddenException(
        'Solo Directores, Administradores y Sistemas pueden ajustar liquidaciones.',
      );
    }

    if (liquidacion.estado !== EstadoLiquidacion.APROBADA) {
      throw new ForbiddenException(
        'Solo se pueden ajustar liquidaciones en estado APROBADA.',
      );
    }
  }
  ...
  validarModificacionTotal(liquidacion: Liquidacion, user: User) {
    if (
      ...
      throw new ForbiddenException(
        'Solo Directores, Administradores y Sistemas pueden modificar el total a pagar.',
      );
    }

    if (liquidacion.estado !== EstadoLiquidacion.APROBADA) {
      throw new ForbiddenException(
        'Solo se pueden modificar totales en liquidaciones APROBADAS.',
      );
    }
  }
}
