...
export class GastoCombustibleService {
  ...
  async create(createGastoCombustibleDto: CreateGastoCombustibleDto, user: User ,file?: Express.Multer.File ) {
    ...
    return await this.dataSource.transaction(async manager => {
      ...
      const litros = Number((createGastoCombustibleDto.monto / createGastoCombustibleDto.precio_litro).toFixed(2));
  ...
  async findOne(id: number) {
    ...
    if (!gastoCombustible) throw new NotFoundException(`El gasto de combustible con el ID: ${id} no fue encontrado`)
  ...
  async update(id: number, updateGastoCombustibleDto: UpdateGastoCombustibleDto, user: User ,file?: Express.Multer.File) {
    ...
    return await this.dataSource.transaction(async manager => {
      ...
      if (!gastoCombustible) throw new NotFoundException('El gasto de combustible no fue encontrado');
      
      validarBloqueoEdicion(gastoCombustible.liquidacion, user);

      if(file) {
        if(gastoCombustible.evidencia_public_id){
          await this.uploadImageService.deleteFile(gastoCombustible.evidencia_public_id);
        }
      ...
      const precio_litro = updateGastoCombustibleDto.precio_litro ?? gastoCombustible.precio_litro;
