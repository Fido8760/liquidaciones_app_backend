...
export class UsersService {
  ...
  async update(id: number, updateUserDto: UpdateUserDto, user: User) {
    ...
    if (userToEdit.email === 'soporte@mudanzasamado.mx') {
      throw new ForbiddenException(
        'Este Usuario Maestro no puede ser modificado ni desactivado',
      );
    }
    ...
    if (updateUserDto.nombre)
      updateUserDto.nombre = updateUserDto.nombre.trim();
    if (updateUserDto.apellido)
      updateUserDto.apellido = updateUserDto.apellido.trim();
  ...
  async remove(id: number, user: User) {
    ...
    if (userRemove.email === 'soporte@mudanzasamado.mx') {
      throw new ForbiddenException(
        'Este Usuario Maestro no puede ser eliminado ni desactivado',
      );
    }
  ...
  async cambiarEstado(
    id: number,
    cambiarEstadoUsuarioDTO: CambiarEstadoUsuarioDTO,
    user: User,
  ): Promise<User> {
    ...
    if (usertToUpdate.email === 'soporte@mudanzasamado.mx') {
      throw new ForbiddenException(
        'Este Usuario Maestro no puede ser desactivado',
      );
    }
