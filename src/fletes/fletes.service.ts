...
export class FletesService {
  constructor(
    ...
    private readonly fleteRepository: Repository<Flete>,
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (!flete) {
        throw new NotFoundException(
          `El costo del flete con ID ${id} no existe`,
        );
      }

      validarBloqueoEdicion(flete.liquidacion, user);

      if (updateFleteDto.cliente)
        updateFleteDto.cliente = updateFleteDto.cliente.trim().toUpperCase();
      if (updateFleteDto.origen)
        updateFleteDto.origen = updateFleteDto.origen.trim().toUpperCase();
      if (updateFleteDto.destino)
        updateFleteDto.destino = updateFleteDto.destino.trim().toUpperCase();
      if (updateFleteDto.descripcion)
        updateFleteDto.descripcion = updateFleteDto.descripcion.trim();
